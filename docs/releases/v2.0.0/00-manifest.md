...
이 문서는 학습 도구 앱 기획 문서 시스템의 장부다. 이 디렉터리는 v2.0.0 시점의 전체 문서 스냅샷이다. 이후 minor/patch 변경은 `docs/changes/v2/`에 증분 기록으로 남긴다.
...
| `04-ux-spec.md` | UX 설계 | Draft | 화면 구성, DAG 카드/리스트, 조작 패널, 테스트 화면 |
| `05-architecture.md` | 기술 아키텍처 | Draft | reducer, validation, storage, event log, graph layout state |
