...
Owns: 구현 구조, reducer, validation, storage, event log, canvas state
Depends On:
  - `02-domain-model.md`
...
무한캔버스의 공간 상태는 DAG의 의미론적 edge와 분리한다.
...
1. 노드 드래그는 `nodePositions`만 바꾼다.
2. 노드 드래그는 선수관계 edge를 바꾸지 않는다.
3. edge 수정은 반드시 GraphPatch 또는 명시적 edge action으로만 수행한다.
4. canvas state는 프로젝트와 함께 저장하되, domain validation의 대상은 아니다.
5. 자동 레이아웃은 초기 배치와 재배치 제안만 담당한다.
...
type ProjectAction =
  ...
  | { type: 'UPDATE_CANVAS_STATE'; payload: CanvasState }
...
| `projects:{projectId}:canvas` | CanvasState |
...
- canvas state는 의미론적 DAG와 분리 저장 가능하게 둔다.
...
| canvas layout 실패 | fallback card list 표시 |
