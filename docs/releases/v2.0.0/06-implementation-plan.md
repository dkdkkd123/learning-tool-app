...
MVP의 목표는 “캡스톤 중심 학습 프로젝트 하나를 만들고, 역추적 DAG를 카드 리스트에서 탐색하며, 노드별 이해 테스트를 통해 학습 진행과 성취 로그를 완성하는 것”이다.
...
## Phase 2 — DAG Draft + Card List Visualization
...
- DAG가 계층형 카드 리스트로 표시된다.
- 노드를 선택하면 상세 정보가 표시된다.
...
| DAG가 너무 복잡해짐 | ready node 중심 리스트, prerequisite chips, refactor command 제공 |
